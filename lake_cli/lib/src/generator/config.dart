...
class GeneratorConfig {
  ...
  static Future<GeneratorConfig?> load([String dir = '']) async {
    final serverPackageDirectoryPathParts = path.split(dir);

    Map<dynamic, dynamic> readYaml(String path) {
      final pubspecFile = File(path);
      final yamlString = pubspecFile.readAsStringSync();
      final result = loadYaml(yamlString) as Map<dynamic, dynamic>;

      return result;
    }

    final pubspecPath = path.join(dir, 'pubspec.yaml');
    final pubspec = readYaml(pubspecPath);

    final nameValue = pubspec['name'] as String?;

    if (nameValue == null) {
      throw const FormatException('Package name is missing in pubspec.yaml');
    }

    final name = _stripPackage(nameValue);

    final configGeneratorPath = path.join(dir, 'config', 'generator.yaml');
    final generatorConfig = readYaml(configGeneratorPath);

    final typeValue = generatorConfig['type'] as String;
    ...
    // final relativeDartClientPackagePathParts =
    //     path.split(generatorConfig['client_package_path']) as String;
    ...
    //     final Map modulesData = generatorConfig['modules'];
    //     for (final package in modulesData.keys) {
    ...
    // // Autodetect modules.
    // final automagicModules = await locateModules(
    //   directory: Directory(dir),
    //   exludePackages: [serverPackage],
    // );
    // for (final autoModule in automagicModules) {
    ...
    // // Load extraClasses
    // final extraClasses = <TypeDefinition>[];
    // if (generatorConfig['extraClasses'] != null) {
    //   try {
    //     for (final extraClassConfig in generatorConfig['extraClasses']) {
    //       extraClasses.add(
    //         parseAndAnalyzeType(
    //           extraClassConfig,
