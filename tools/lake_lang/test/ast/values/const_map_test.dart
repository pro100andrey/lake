...
void main() {
  group('ConstMap AST', () {
    test('should parse empty constant map', () {
      ...
      final constMap = field.defaultValue!.cast<ConstMapNode>();
    ...
    test('should parse constant map with string keys and string values', () {
      ...
      final [entry1, entry2] = constMap.entries
          .cast<({LiteralNode key, LiteralNode value})>();
    ...
    test('should parse constant map with integer keys and boolean values', () {
      ...
      final [entry0, entry1] = constMap.entries
          .cast<({IntConstantNode key, BoolConstantNode value})>();
    ...
    test('should parse constant map with nested constant map as value', () {
      ...
      final [entry0] = constMap.entries
          .cast<({LiteralNode key, ConstMapNode value})>();
      ...
      final [ne1, ne2] = nestedMap.entries
          .cast<({LiteralNode key, AstNode value})>();

      expect(ne1.key.cast<LiteralNode>().rawValue, '"id"');
      expect(ne1.key.cast<LiteralNode>().value, 'id');
      expect(ne1.key.span, hasSpan(62, 66));

      expect(ne1.value.cast<IntConstantNode>().rawValue, '123');
      expect(ne1.value.cast<IntConstantNode>().value, 123);
      expect(ne1.value.span, hasSpan(68, 71));

      final nestedEntry2 = nestedMap.entries[1];
      expect(nestedEntry2.key.cast<LiteralNode>().rawValue, '"active"');
      expect(nestedEntry2.key.cast<LiteralNode>().value, 'active');
      expect(nestedEntry2.key.span, hasSpan(73, 81));

      expect(nestedEntry2.value.cast<BoolConstantNode>().rawValue, 'true');
      expect(nestedEntry2.value.cast<BoolConstantNode>().value, true);
    ...
    test('should parse constant map with mixed keys and values', () {
      ...
      expect(entry0.key.cast<LiteralNode>().rawValue, '"count"');
      expect(entry0.key.cast<LiteralNode>().value, 'count');
      expect(entry0.key.span, hasSpan(38, 45));

      expect(entry0.value.cast<IntConstantNode>().rawValue, '5');
      expect(entry0.value.cast<IntConstantNode>().value, 5);
      expect(entry0.value.span, hasSpan(47, 48));

      expect(entry1.key.cast<IntConstantNode>().rawValue, '100');
      expect(entry1.key.cast<IntConstantNode>().value, 100);
      expect(entry1.key.span, hasSpan(50, 53));

      expect(entry1.value.cast<LiteralNode>().rawValue, '"score"');
      expect(entry1.value.cast<LiteralNode>().value, 'score');
      expect(entry1.value.span, hasSpan(55, 62));

      expect(entry2.key.cast<BoolConstantNode>().rawValue, 'true');
      expect(entry2.key.cast<BoolConstantNode>().value, true);
      expect(entry2.key.span, hasSpan(64, 68));

      expect(entry2.value.cast<BoolConstantNode>().rawValue, 'false');
      expect(entry2.value.cast<BoolConstantNode>().value, false);
  ...
  group('ConstMap AST (equable)', () {
    test('should be equal for identical constant maps', () {
      ...
      final map1 = struct1.fields.first.defaultValue!.cast<ConstMapNode>();
      final map2 = struct2.fields.first.defaultValue!.cast<ConstMapNode>();
